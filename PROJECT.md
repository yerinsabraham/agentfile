...
| **[r2] A missing template fails the build via the import itself** | Revision 1 promised this with no mechanism. Static imports supply it: delete a template and `next build` fails at compile. No custom check script, and nothing to remember to run. |

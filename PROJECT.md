...
| **[r5] Two webfonts, self-hosted via `next/font/google`** | Instrument Serif for the page title and Inter for everything else, matching Creovine Academy's tokens exactly. Lesson 3 puts this on screen for three minutes and beginners copy what they see, so shipping the default `system-ui` and `#1d4ed8` of every generated app teaches the wrong thing. `next/font/google` is built into Next, so `package.json` stays at three dependencies and §1 is untouched. **This revision replaced a stylesheet link.** That version cost a third-party request on every page load and needed a `no-page-custom-font` lint suppression, and this one has neither: the files are downloaded during `next build`, emitted into `out/`, and served from this site. Verified after the change: zero references to `fonts.googleapis.com` or `fonts.gstatic.com` anywhere in the build, nine `.woff2` files emitted, `@font-face` pointing at `../media/`. The argument that an explicit `<link>` teaches a beginner more was wrong: it teaches a pattern they would later unlearn, plus a disabled lint rule explaining why the warning does not apply. One cost remains and is accepted: the build now needs network access the first time, to fetch the fonts. |

...
| **[r5] Two webfonts, loaded from Google Fonts by stylesheet link** | Instrument Serif for the page title and Inter for everything else, matching Creovine Academy's tokens exactly. Lesson 3 puts this on screen for three minutes and beginners copy what they see, so shipping the default `system-ui` and `#1d4ed8` of every generated app teaches the wrong thing. A `<link>` rather than an npm package keeps `package.json` at three dependencies, which §1 protects. Two known costs, accepted: the page now makes a request to a third party it did not make before, and a font that fails to load falls back rather than blocking. Neither touches the no-backend rule, since both are browser requests and nothing runs on a server. |
